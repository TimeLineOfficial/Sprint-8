...
export const Navbar = ({ cartCount = 0, theme, toggleTheme, isOnline }) => {
  ...
  const navLinks = [
    ...
    { path: '/checkout', label: 'Checkout & Cart' },
    { path: '/performance-audit', label: 'Platform Performance' }
  ];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        ...
        <Link to="/" className="flex items-center space-x-2.5">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-[9px] font-medium text-slate-400 uppercase tracking-wider mt-0.5">
              Global Products &amp; Certified Hardware Store
            </span>
          </div>
        </Link>
        ...
        <div className="flex items-center space-x-3 text-xs">
          ...
          <div className={`px-2.5 py-1 rounded-full text-[10px] font-bold flex items-center gap-1.5 border ${
            ...
            <span className="hidden sm:inline">{isOnline ? 'Online' : 'Offline Cache Mode'}</span>
          </div>
